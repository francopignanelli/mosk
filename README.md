...
A digital terrarium by **Franco Pignanelli** for visually and interactively exploring published fruit-fly brain research and the limits of simulating behavior. An autonomous virtual fly roams an endlessly generated world alongside a source-backed research workbench. **The roaming controller is an artificial sensory utility model, not the MaleCNS connectome.** Research & learning adds real MaleCNS anatomical records and a separate published mushroom-body memory assay. The assay does not steer the roaming fly.
...
The camera follows the fly at every zoom level. Drag the map or focus it and use arrow keys to pan; press **F** or the crosshair to return to the fly. Soft exploration fog opens around the animal and remembers visited places across reloads. Panning never generates or reveals new territory. Naturally generated spiders are drawn only within the fly's current sight range, so old explored terrain does not reveal distant predator activity. Observer-placed spiders remain visible in revealed, active terrain so an intervention can be inspected; they are drawn faintly outside their detection range or while the fly is hidden in dense cover. Visibility on the observer's map does not give the fly extra sensory range. A sector counts as explored when the fly enters it, and first visits appear in the journal. Discovery is an observer record, not a cognitive map available to the controller. Old saves reconstruct initial fog coverage from the current position and retained trail; older travel was not recorded precisely enough to reconstruct full historical visibility.
...
Food, Water and Spider map brushes let an observer place stimuli by clicking or dragging through explored, active terrain; Move map returns to camera panning. Placements enter the same local world as other resources and predators, are recorded and saved with the experiment, and can be used for controlled before-and-after observations. An added spider uses the same pursuit update as a generated spider. Place one within its approximately 145-unit detection range, outside dense refuge cover, to test an immediate encounter: if no chase is active, that deliberate placement can start one even during the shared recovery interval. Beyond detection range or while the fly is sheltered, the spider roams until it can detect the fly. Observer visibility does not imply fly perception. These interventions do not train the separate published memory assay or turn the utility controller into a connectome model.
...
The top-bar **Export JSON** button downloads a portable backup of the current life and archive. **Import JSON** selects one of those files for restoration on this device. Import validates the version, types, finiteness, ranges and entity limits before writing; a preview explains that it replaces this device's current experiment and archive. A failed storage commit leaves the current state intact. Imported runs start paused. Files are limited to 50 MB. Normal use requires neither button because autosave and reload happen locally. Browser storage is device- and origin-local, not cloud backup; clearing it removes experiments. Export important records before changing devices or importing another run. The growing archive is intentionally not silently pruned; a future release should move individual histories to separate IndexedDB records for long-running studies.
